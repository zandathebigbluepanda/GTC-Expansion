...
public class CoverStorage implements INetworkFieldData {
    ...
    public CoverStorage(GTCXTileBasePipe owner) {
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            coverLogicMap.put(facing, new GTCXNullLogic(owner, facing));
        }
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            EnumFacing facing = EnumFacing.VALUES[i]; // Use EnumFacing.VALUES directly
            this.coverLogicMap.put(facing, logicFromInt(data.getInteger("Type"), facing));
            this.coverLogicMap.get(facing).readFromNBT(data);
        }
    }
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < 6; i++) {
            NBTTagCompound data = new NBTTagCompound();
            data.setInteger("Type", intFromLogic(coverLogicMap.get(EnumFacing.VALUES[i]))); // Use EnumFacing.VALUES directly
            this.coverLogicMap.get(EnumFacing.VALUES[i]).writeToNBT(data); // Use EnumFacing.VALUES directly
    ...
    public QuadList[] getQuadList() {
        ...
        for (int i = 0; i < 6; ++i) {
            list[i] = this.getQuads(EnumFacing.VALUES[i]); // Use EnumFacing.VALUES directly
        }
    ...
    public void read(IInputBuffer buffer) {
        ...
        coverLogicMap = new LinkedHashMap<>();
        for (int i = 0; i < 6; i++) {
            this.coverLogicMap.put(EnumFacing.VALUES[i], logicFromInt(buffer.readInt(), EnumFacing.VALUES[i])); // Use EnumFacing.VALUES directly
            this.coverLogicMap.get(EnumFacing.VALUES[i]).read(buffer); // Use EnumFacing.VALUES directly
        }
    }
    ...
    public void write(IOutputBuffer buffer) {
        ...
        for (int i = 0; i < 6; i++) {
            buffer.writeInt(intFromLogic(coverLogicMap.get(EnumFacing.VALUES[i]))); // Use EnumFacing.VALUES directly
            this.coverLogicMap.get(EnumFacing.VALUES[i]).write(buffer); // Use EnumFacing.VALUES directly
        }
    }
    ...
    protected GTCXBaseCoverLogic logicFromInt(int value, EnumFacing facing) {
        switch (value) {
            case 1:
                return new GTCXConveyorModuleLogic(owner, facing);
            case 2:
                return new GTCXDrainModuleLogic(owner, facing);
            case 3:
                return new GTCXItemValveModuleLogic(owner, facing);
            case 4:
                return new GTCXPumpModuleLogic(owner, facing);
            case 5:
                return new GTCXShutterLogic(owner, facing);
            case 6:
                return new GTCXRedstoneControllerLogic(owner, facing);
            case 7:
                return new GTCXFluidFilterLogic(owner, facing);
            default:
                return new GTCXNullLogic(owner, facing);
        }
    }
